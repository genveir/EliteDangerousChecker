...
public record Species(long Id, string Name) : IIdAndNameTableEntity { }

public static class SpeciesAccess
{
    ...
    {
        Items = await IdAndNameTableAccess.GetItems<Species>(tableName: "Species");
    }

    private static async Task AddItem(long id, string value) => await IdAndNameTableAccess.AddItem<Species>(tableName: "Species", id, value);
}
