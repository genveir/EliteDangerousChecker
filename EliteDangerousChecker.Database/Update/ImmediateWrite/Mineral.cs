...
public record Mineral(long Id, string Name) : IIdAndNameTableEntity { }

public static class MineralAccess
{
    ...
    {
        Items = await IdAndNameTableAccess.GetItems<Allegiance>(tableName: "Mineral");
    }

    private static async Task AddItem(long id, string value) => await IdAndNameTableAccess.AddItem<Mineral>(tableName: "Mineral", id, value);
}
