...
public record Allegiance(long Id, string Name) : IIdAndNameTableEntity { }

public static class AllegianceAccess
{
    ...
    {
        Items = await IdAndNameTableAccess.GetItems<Allegiance>(tableName: "Allegiance");
    }

    private static async Task AddItem(long id, string value) => await IdAndNameTableAccess.AddItem<Allegiance>(tableName: "Allegiance", id, value);
}
